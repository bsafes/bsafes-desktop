  ...
  var handleDropAction = function(e) {
	...
	function displayTeams(teams) {
	  ...
	  function isTeamSelected(teamId) {
        ...
        thisTeamNameHTML = "<h2>" + thisTeamName + "</h2>";
				thisTeamNameHTML = DOMPurify.sanitize(thisTeamNameHTML);
      	$resultItem.find('.itemTitle').html(thisTeamNameHTML);
        ...
        if (data.hits.total > itemsPerPage) {
          $.post(server_addr + '/memberAPI/listTeams', {
            ...
            if (total_data.status === 'ok') {
              ...
              console.log('err:(/memberAPI/listTeams)', total_data);
            }
          })
        } else {
          dbInsertTeamList(server_addr + '/memberAPI/listTeams', data);
        }
