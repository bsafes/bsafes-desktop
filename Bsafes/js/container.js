function loadPage(){
  ...
  function getItem(done, thisVersion) {
    ...
    , function(data, textStatus, jQxhr ){
      if(data.status === 'ok') {
        if(data.item) {	
          dbInsertContainers(server_addr + '/memberAPI/getItemData', itemId, data);
					itemCopy = data.item;
					if(!thisVersion) {
            setCurrentVersion(itemCopy.version);
          } else {
...
/* For notebook */
  ...
  var goToPage = function(e) {
    ...
    window.location.href = makeCallNavigate('/notebook/p/' + intendedItemId);  
    return false;
  };
...
/* End of diary*/
  ...
  function searchByTokens(searchTokensStr, pageNumber) {
    $.post(server_addr + '/memberAPI/searchInsideContainer', {
			container: itemId,
        ...
        if (data.hits.total > itemsPerPage) {
          $.post(server_addr + '/memberAPI/getContainerContents', {
            ...
            if(total_data.status === 'ok') {
              ...
              console.log('err:(/memberAPI/getContainerContents)', total_data);
            }
          })
        } else {
          dbInsertContainers(server_addr + '/memberAPI/getContainerContents', itemId, data);
        }
  ...
  $('#listAllItems').click(listAllItems);
    ...
    $('#gotoContentsBtn').trigger('click');
 	...
 	$('#nextPageBtn').click(function(e) {
      ...
      window.location.href = makeCallNavigate(firstResultItemLink);    
		}
    return false;
  });
