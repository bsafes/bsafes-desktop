...
function showPath(
  ...
  for (var i = 0; i < path.length; i++) {
    $pathItem = $(".pathItemTemplate")
      .clone()
      .removeClass("pathItemTemplate hidden")
      .addClass("pathItem");
    ...
    var $newTab = $(".newTabTemplate")
      .clone()
      .removeClass("newTabTemplate hidden")
      .addClass("newTab");
    ...
    if (i === 0) {
      ...
      $pathItem
        .find(".itemInSamePage")
        .html(pathItemIcon + " " + itemTitleText);
      //$pathItem.find('.itemInSamePage').attr('href', pathItemLink);
      $pathItem
        .find(".itemInSamePage")
        .attr("href", makeCallNavigate(pathItemLink));
  ...
  var $newTab = $(".newTabHintTemplate")
    .clone()
    .addClass("newTabHint")
    .removeClass("hidden");
  $(".newTabDropdownMenu").prepend($newTab);

  $pathItemName = $(".pathItemNameTemplate")
    .clone()
    .removeClass("pathItemNameTemplate hidden")
    .addClass("pathItemName");
  $pathItemName.text("");
...
function showPleaseWait() {
  if (document.querySelector("#pleaseWaitDialog") == null) {
    var modalLoading =
      '<div class="modal" id="pleaseWaitDialog" data-backdrop="static" data-keyboard="false" role="dialog" style="position:fixed">\
...
function lockBSafes(why) {
  ...
  window.location.href = makeCallNavigate(
    "/lockBSafes?redirectURL=" + thisUrl + "&why=" + why
  );
}
...
function initializeItemVersionsHistory(itemId, getItemVersion) {
  ...
  $("#itemVersionsHistory").click(function(e) {
    $.post(
      server_addr + "/memberAPI/getItemVersionsHistory",
      {
        itemId: itemId,
        size: 20,
        from: 0
      ...
      function(data, textStatus, jQxhr) {
        if (data.status === "ok") {
          ...
          for (var i = 0; i < hits.length; i++) {
            var $itemVersionItem = $(".itemVersionItemTemplate")
              .clone()
              .removeClass("itemVersionItemTemplate hidden")
              .addClass("itemVersionItem");
            $itemVersionItem.attr("id", hits[i]._source.version);
            $itemVersionItem
              .find(".itemVersion")
              .html("V." + hits[i]._source.version);
            var updatedText;
            ...
            var updatedBy = hits[i]._source.displayName
              ? hits[i]._source.displayName
              : hits[i]._source.updatedBy;
            ...
            if (updatedTime.charAt(updatedTime.length - 1) === "o") {
              $itemVersionItem
                .find(".itemVersionUpdatedTimeStamp")
                .text(timeToString(hits[i]._source.createdTime));
            }
            $(".itemVersionItemsList").append($itemVersionItem);
            $itemVersionItem.click(function(e) {
              $("#itemVersionsModal").modal("hide");
              var $thisItemVersionItem = $(e.target).closest(
                ".itemVersionItem"
              );
...
function initContainerFunctions(
  listItems,
  searchByTokens,
  decryptResult,
  updateToolbar,
...
function displayHits(mode, hits, options) {
  ...
  for (var i = 0; i < hits.length; i++) {
    ...
    if (options && options.inTrash) {
      $resultItem.data(
        "originalContainer",
        resultItem._source.originalContainer
      );
      if (resultItem._source.originalPosition) {
        $resultItem.data(
          "originalPosition",
          resultItem._source.originalPosition
        );
      } else {
...
function updatePagination(
  ...
  if ($containerContentsPagination.hasClass("hidden")) {
    ...
    for (var i = 0; i < numberOfContentsPages; i++) {
      ...
      $newItem.find("a").click(function(e) {
        e.preventDefault();
        var intendedPageNumber = parseInt(
          $(e.target)
            .parent()
            .attr("id")
        );
  ...
  $(".containerContentsPagination")
    .find("li.disabled")
    .removeClass("disabled");
  $(".containerContentsPagination")
    .find("li#" + currentContentsPage)
    .addClass("disabled");
}
...
var handleDropAction = function(e) {
  ...
  $.post(
    server_addr + "/memberAPI/" + dropAction,
    {
      space: currentSpace,
...
function newResultItem(resultItem) {
  ...
  var displayName = title.replace(/<[^>]*>?/gm, "");
  ...
  var $resultItem = $(".resultItemTemplate")
    .clone()
    .removeClass("resultItemTemplate hidden")
    .addClass("resultItem");
  ...
  switch (itemType) {
    case "p":
      ...
      title =
        '<i class="fa fa-file-text-o safeItemTypeIcon" aria-hidden="true"></i>' +
        title;
    ...
    case "b":
      title =
        '<i class="fa fa-archive safeItemTypeIcon" aria-hidden="true"></i>' +
        title;
      link = "/box/" + id;
      break;
    case "f":
      title =
        '<i class="fa fa-folder-o safeItemTypeIcon" aria-hidden="true"></i>' +
        title;
      link = "/folder/" + id;
      break;
    case "n":
      title =
        '<i class="fa fa-book safeItemTypeIcon" aria-hidden="true"></i>' +
        title;
    ...
    case "d":
      title =
        '<i class="fa fa-calendar safeItemTypeIcon" aria-hidden="true"></i>' +
        title;
  ...
  if (itemType === "np") {
    ...
    $resultItem.find(".itemPage").attr("href", makeCallNavigate(link));
  }
  if (itemType === "dp") {
  ...
  $resultItem.find(".itemLink").attr("href", makeCallNavigate(link));
...
function createANewItem(
  ...
  $.post(
    server_addr + "/memberAPI/" + addAction,
    addActionOptions,
    function(data, textStatus, jQxhr) {
...
function listContainers(itemId, itemTitle) {
  ...
  $.post(
    server_addr + "/memberAPI/listContainers",
    {
      container: itemId,
    ...
    function(data, textStatus, jQxhr) {
      if (data.status === "ok") {
        ...
        var $moveItemsPathItem = $(".moveItemsPathItemTemplate")
          .clone()
          .removeClass("moveItemsPathItemTemplate hidden")
          .addClass("moveItemsPathItem");
...
function displayContainers(containers, total, mode) {
  ...
  if (containers.length) {
    for (var i = 0; i < containers.length; i++) {
      ...
      if (thisItem.type === "B") {
        var $thisItem = $(".boxTemplate")
          .clone()
          .removeClass("boxTemplate hidden")
          .addClass("box");
      } else if (thisItem.type === "F") {
        var $thisItem = $(".folderTemplate")
          .clone()
          .removeClass("folderTemplate hidden")
          .addClass("folder");
      }
      var itemKey = decryptResultInContainer(
        thisItem.keyEnvelope,
        thisItem.envelopeIV
      );
      var itemIV = decryptResultInContainer(
        thisItem.ivEnvelope,
        thisItem.ivEnvelopeIV
      );
      if (thisItem.title) {
        try {
          var encodedTitle = decryptBinaryString(
...
function showMoveItemsModal(thisSpace) {
  ...
  $("#dropItemsBtn").click(function(e) {
    ...
    console.log(
      ...
      currentTargetContainerName
    );
    showLoadingInMoveItemsModal();
    $.post(
      server_addr + "/memberAPI/dropItemsInside",
      {
        space: currentSpace,
        items: JSON.stringify(selectedItemsInContainer),
...
function showTrashItemsModal(thisSpace, originalContainer) {
  ...
  $thisBtn.click(function(e) {
    ...
    $.post(
      server_addr + "/memberAPI/trashItems",
      {
        items: JSON.stringify(selectedItemsInContainer),
...
(function() {
  ...
  function positionPageControls() {
    ...
    if ($pagePanel.length) {
      ...
      $(".btnFloatingWrite, .btnFloatingSave, .btnFloatingCancel").css(
        "right",
        rightMargin + "px"
      );
      /*
...
async function downloadItemByItemID(arrList, done) {
  ...
  if (itemType == "page") {
    ...
    $.post(server_addr + "/memberAPI/getPageItem", { itemId: itemID }, function(
      ...
      dbQueryGetPageItem(
        server_addr + "/memberAPI/getPageItem",
        { itemId: itemID },
