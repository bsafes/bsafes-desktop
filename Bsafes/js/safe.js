function loadPage() {
    ...
    $('#teamsBtn').click(function(e) {
        ...
        window.location.href = makeCallNavigate('/teams/');
        return false;
    });
    ...
    function searchByTokens(searchTokensStr, pageNumber) {
        $.post(server_addr + '/memberAPI/search', {
                ...
                if (data.hits.total > itemsPerPage) {
                  $.post(server_addr + '/memberAPI/listItems', {
                    ...
                    if (total_data.status === 'ok') {
                      ...
                      console.log('err:(/memberAPI/listItems)', total_data);
                    }
                  })
                } else {
                  dbInsertTeams(server_addr + '/memberAPI/listItems', teamId, data);
                }
            ...
            if (teamId.substring(0, 1) === 't') {
                ...
                getTeamData(teamId, function(err, team) {
                    if (err) {
                        ...
                        $newTab.find('a').attr('href', makeCallNavigate('/team/' + teamId));
                ...
                $newTab.find('a').attr('href', makeCallNavigate('/safe'));
                $newTab.insertAfter($("#newTabHint"));
            }
        }
    });
