function loadPage(){
    ...
    addActionOptions = {
      ...
      function(data, textStatus, jQxhr) {
        if(data.status === 'ok') {
          ...
          window.location.href = makeCallNavigate('/folder/p/' + item.id);
        }
      }, 'json');
	};
  ...
  $('#gotoCoverBtn').click(function(e){
    ...
    window.location.href = makeCallNavigate('/folder/' + folderId + '?initialDisplay=cover');
    return false;
  });
    ...
    window.location.href = makeCallNavigate('/folder/' + folderId + '?initialDisplay=contents');
    return false;
  });
  ...
  $('#gotoLastItemBtn').click(function(e){
	...
	$('#nextPageBtn').click(function(e){
		...
		$.post(server_addr + '/memberAPI/getNextFolderPage', {
			folderId: folderId,
			itemId: itemId,
    ...
    $.post(server_addr + '/memberAPI/getPreviousFolderPage', {
      folderId: folderId,
      itemId: itemId,
      itemPosition: itemPosition
    }, function(data, textStatus, jQxhr) {
