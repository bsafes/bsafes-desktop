...
function loadPage(){
	...
	$('#startBtn').click(function(e) {
      ...
      return;
		...
		$.post(server_addr + '/registerEmail', {
			email: email
	...
	$('#sendCodeBtn').click(function(e) {
		...
		$.post(server_addr + '/resetEmailPasswordVerifyCode', {
  ...
  $('#sendPasswordBtn').click(function(e) {
    $.post(server_addr + '/resetEmailPassword', {
			password: password 
    }, function(data, textStatus, jQxhr ){
      if(data.status === 'ok') {
				//window.location.href = "/member/";
        window.location.href = makeCallNavigate("/member/");
      }
    }, 'json');
    return false;
  });
