/*-- Commons for all item views */
    ...
    dbQueryItemPath(server_addr + '/memberAPI/getItemPath', {
      itemId: itemId
    ...
    $('.pathItem, .pathItemName').remove();
   	...
   	for(var i=0; i< path.length; i++) {
        ...
        $pathItem.find('.itemInSamePage').attr('href', makeCallNavigate(pathItemLink));
  ...
  function hideLoadingInTrashBox() {
    ...
    window.location.href = makeCallNavigate("/lockBSafes?redirectURL="+thisUrl + "&why="+why) ;   
	};
...
/*-- Commons for container views */
  ...
  var handleDropAction = function(e) {
    ...
    $.post(server_addr + '/memberAPI/' + dropAction, {
			space: currentSpace,
...
function getTeamData(teamId, done) {
  ...
  dbQueryGetTeamData(server_addr + '/memberAPI/getTeamData', {
		//teamId: teamId
    itemId: teamId
	}, function(data, textStatus, jQxhr) {
		if(data.status === 'ok') {
